<template>
  <div><div class="container is-fluid">
    ...
    <div class="columns main-columns">
      <div class="column is-3">
        <ul class="services">
          <li v-for="(service, index) in services">
            <span @click="selectService(index)" class="service-name">
              {{ service.name }}, {{ service.duration }} minutes, ${{service.price}} <i class="fas fa-edit"></i>
            </span>
            <ul class="practitioners">
              <li v-for="practitioner in service.practitioners">{{practitioner.firstName}} {{practitioner.lastName}}</li>
            </ul>
          </li>
        </ul>
      </div>
      <div class="column is-6" v-if="selectedService">
        <div class="edit-column">
          <div class="service-header">
            ...
            <span class="right-align has-text-danger delete-button" @click="deleteService(selectedService.id)">
          ...
          <div class="edit-form">
            ...
            <div class="field">
              ...
              <div class="field has-addons">
                <div class="control is-expanded">
                  <input class="input" v-model="selectedService.duration" type="number" placeholder="Service duration">
                </div>
                <p class="control">
            ...
            <div class="field is-grouped">
              <div class="control">
                <button @click="updateService" class="button is-link">Save</button>
              </div>
              <div class="control">
      ...
      <div class="column is-2" v-if="selectedService">
  ...
  export default {
    ...
    computed: {
      offeringPractitioners() {
        if (this.selectedServiceIndex !== null) {
          return this.services[this.selectedServiceIndex].practitioners.map(p => p.id)
        }
      }
    },
    created() {
    ...
    methods: {
      ...
      selectService(index) {
        ...
        console.log("Selecting service: " + index)
        ...
        if (index === null) {
          ...
          this.setService(this.services[this.selectedServiceIndex])
        }
      },
      getServices() {
        let self = this;
